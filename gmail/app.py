...
from flask import Flask, Response, has_request_context, jsonify, redirect, request, send_file, session, url_for
...
def simulate_website_preview(urls: List[str]) -> Dict[str, Any]:
    ...
    try:
        ...
        parsed_url = urlparse(response.url or target_url)
        lowered_content = content.lower()
        has_password = bool(re.search(r'type=["\']password["\']', content, re.IGNORECASE))
        has_form = "<form" in lowered_content
        keyword_flags = [
            keyword for keyword in ["login", "verify", "bank", "password", "sign in", "otp"]
            if keyword in lowered_content
        ]
        risk_score = min(100, (45 if has_password else 0) + (15 if has_form else 0) + (len(keyword_flags) * 10))
        suspicious = has_password or ("login" in lowered_content and has_form) or len(keyword_flags) >= 2
...
def get_archive_connection() -> Optional[Any]:
    ...
    if pymysql is None:
        LAST_DB_ERROR = "pymysql is not installed"
        return None
    try:
        LAST_DB_ERROR = ""
...
def init_archive_db() -> None:
    ...
    if connection is None:
        print(f"MySQL archive unavailable: {LAST_DB_ERROR or 'connection failed'}")
        return
    print(f"MySQL archive connected: {MYSQL_USER}@{MYSQL_HOST}:{MYSQL_PORT}/{MYSQL_DATABASE}.{MYSQL_TABLE}")
    connection.close()


def load_history_from_db() -> None:
    """Load archive rows from the existing MySQL threat_history table."""
    existing_rows = list(ANALYSIS_HISTORY)
    connection = get_archive_connection()
    if connection is None:
        if LAST_DB_ERROR:
            print(f"MySQL read skipped: {LAST_DB_ERROR}")
        return
    ...
    seen_keys = set()
    for record in loaded_rows + existing_rows:
        key = record.get("unique_key") or (
            f"{record.get('source_type', 'gmail')}|"
            f"{record.get('timestamp', '')}|"
            f"{record.get('sender', '')}|"
            f"{record.get('message_text', '')}"
        )
        if key in seen_keys:
            continue
        seen_keys.add(key)
...
def add_history(entry: Dict[str, Any]) -> None:
    ...
    entry["unique_key"] = cache_key
    DETAIL_CACHE[cache_key] = entry.get("details", {})

    # Always keep the newest scan rows in memory so the archive tab still works
    # even if MySQL is temporarily unavailable or the page is refreshed mid-scan.
    ANALYSIS_HISTORY[:] = [row for row in ANALYSIS_HISTORY if row.get("unique_key") != cache_key]
    ANALYSIS_HISTORY.insert(0, entry)

    connection = get_archive_connection()
    if connection is None:
        if LAST_DB_ERROR:
            print(f"MySQL write skipped: {LAST_DB_ERROR}")
        return
...
def analyze_url() -> Any:
    """Safely preview one URL from the dashboard without opening it in a browser."""
    target_url = (request.args.get("url") or "").strip()
    if not target_url:
        return jsonify({"error": "Missing url parameter"}), 400
    return jsonify(simulate_website_preview([target_url]))
